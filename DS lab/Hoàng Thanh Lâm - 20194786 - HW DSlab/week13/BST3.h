...
void InsertKey(BinarySearchTree *tree,elementtype x)
{
        ...
        {
	  if(strcmp(x.name,((*tree)->key).name)<0) InsertKey(&((*tree)->left),x);
            else InsertKey(&((*tree)->right),x);
        }
}
node* Search(BinarySearchTree tree, char x[] )
{
    if(tree==NULL) return NULL;
    else if(strcmp((tree)->key.name,x)==0) return tree;
    else if(strcmp((tree)->key.name,x)<0)  return Search(((tree)->right),x);
...
node *Delete(BinarySearchTree *tree,char x[])
{
    node *tmp;
    if((*tree)==NULL) printf("Not found\n");
    else if(strcmp(x,(*tree)->key.name)<0) (*tree)->left=Delete(&((*tree)->left),x);
    else if(strcmp(x,(*tree)->key.name)>0) (*tree)->right=Delete(&((*tree)->right),x);
    else
    {
        if(LeftChild(*tree)!=NULL&&RightChild(*tree)!=NULL)
        {
            tmp=FindMin((*tree)->right);
            strcpy((*tree)->key.name,tmp->key.name);
	    strcpy((*tree)->key.screensize,tmp->key.screensize);
	    strcpy((*tree)->key.memoryspace,tmp->key.memoryspace);
	    (*tree)->key.price=tmp->key.price;
            (*tree)->right=Delete(&((*tree)->right),(*tree)->key.name);
        }
        else 
        {
            tmp=*tree;
...
{
  printf("%-20s\t%-10s\t%-10s\t%d\n",p->key.name,p->key.memoryspace,p->key.screensize,p->key.price);
}
